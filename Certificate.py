...
class Certificate:
    ...
    def verify(content, issuing_keyset, tls_version):
        ...
        if(tls_version == "TLS 1.2"):
            start_key_hash = rsa.RSASignature.verify(signature_to_verify, issuing_keyset)
            valid_certificate = (hash_key == start_key_hash)
        else:          
            valid_certificate = ecc.S256EC_signature.verify(hash_key, issuing_keyset.publicKey, ecc.S256Point(int(signature_to_verify[0], 16), int(signature_to_verify[1], 16)), int(signature_to_verify[2], 16))
