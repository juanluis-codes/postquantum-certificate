...
class App:
    ...
    def __init__(self, root):
        self.root = root
        self.root.geometry("445x190")
        self.root.resizable(0, 0)
        self.root.title("Certificados Certlu")
    ...
    def place_items(self):
        ...
        # Buttons
        self.generate_button = tk.Button(self.root, text = "Generar certificado", command = self.generate, cursor="hand1", bg = SOFT_BLUE, fg = WHITE)
        self.generate_button.place(x = 10, y = 150)
        self.verify_button = tk.Button(self.root, text = "Verificar", command = self.verify, cursor="hand1", bg = SOFT_BLUE, fg = WHITE)
        self.verify_button.place(x = 170, y = 150)
        self.generate_issuing_keyset_button = tk.Button(self.root, text = "Generar nuevas claves", command = self._generate_issuing_keys_file, cursor="hand1", bg = SOFT_BLUE, fg = WHITE)
        self.generate_issuing_keyset_button.place(x = 260, y = 150)
    ...
    def generate(self):
        if(self.tls_version.get() == "TLS 1.2"):
            certificate = cert.Certificate(self.organization_name.get(), self.organization_country.get(), self.organization_location.get(), self.organization_domain.get(), self.tls_version.get())
            if(certificate.generateFile(ISSUING_KEYSET_RSA) == -1):
                tk.messagebox.showerror(title = "Certificado", message = "La generación del certificado ha fallado")
                return
        elif(self.tls_version.get() == "TLS 1.3" or self.tls_version.get() == "TLS 1.5"):
            certificate = cert.Certificate(self.organization_name.get(), self.organization_country.get(), self.organization_location.get(), self.organization_domain.get(), self.tls_version.get())
            if(certificate.generateFile(ISSUING_KEYSET_ECC if (self.tls_version.get() == "TLS 1.3") else ISSUING_KEYSET_CRYSTALS) == -1):
                tk.messagebox.showerror(title = "Certificado", message = "La generación del certificado ha fallado")
                return
        elif(self.tls_version.get() == "TLS 2.0"):
            certificate = cert.Certificate(self.organization_name.get(), self.organization_country.get(), self.organization_location.get(), self.organization_domain.get(), self.tls_version.get())
            if(certificate.generateFile(None) == -1):
                tk.messagebox.showerror(title = "Certificado", message = "La generación del certificado ha fallado")
                return

        tk.messagebox.showinfo(title = "Certificado", message = "La generación del certificado se ha completado satisfactoriamente")
        return
    ...
    def verify(self):
        ...
        if(verified):
            tk.messagebox.showinfo(title = "Verificación", message = "El certificado es correcto y válido")
            return
        else:
            tk.messagebox.showerror(title = "Verificación", message = "El certificado no ha sido verificado")
            return
    ...
    def _generate_issuing_keys_file(self):
        ...
        public_key_text = "RSA\nExponente: " + str(rsa_keyset.getE()) + "\nModulo: " + hex(rsa_keyset.getN())
        private_key_text = "- - - - RSA PRIVATE KEY BEGINS - - - -\n" + hex(rsa_keyset.getD()) + "\n- - - - RSA PRIVATE KEY ENDS - - - -"
        ...
        try:
            ...
            tk.messagebox.showerror(title = "Generación y renovación de claves de firma", message = "La generación y renovación de las claves de firma falló")
            return
        ...
        public_key_text = "ECDSA\n" + str(ecc_keyset.getPublicKey())
        private_key_text = "- - - - ECDSA PRIVATE KEY BEGINS - - - -\n" + hex(ecc_keyset.getPrivateKey()) + "\n- - - - ECDSA PRIVATE KEY ENDS - - - -"
        ...
        private_key_text = "- - - - ECDSA PRIVATE KEY BEGINS - - - -\n" + hex(crystals_keyset.getPrivateKey()) + "\n- - - - ECDSA PRIVATE KEY ENDS - - - -"
        ...
        if(generate_sign_keys_kyber_c.generateKeys() != 0):
            tk.messagebox.showerror(title = "Generación y renovación de claves de firma", message = "La generación y renovación de las claves de firma falló")
            return
        ...
        tk.messagebox.showinfo(title = "Generación y renovación de claves de firma", message = "La generación y renovación de las claves de firma se ha completado correctamente")
        return 
